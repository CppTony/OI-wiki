...
??? +note "[P4231 三步必杀](https://www.luogu.com.cn/problem/P4231)"
    $N$个柱子排成一排，一开始每个柱子损伤度为$0$。接下来进行$M$次攻击，每一次都会使$[l,r]$（l到r的闭区间）的柱子分别遭遇不同的损伤，损伤度从左往右按照一个等差数列从$s$到$e$不等。求得最大的损伤度与所有柱子损伤度的亦或和。
    
??? note "大致思路"
    由于巨大的数据范围，使得进行每次攻击操作都需要常熟级复杂度，这便是差分开始介入的地方。*前缀和支持**常数级别**的查询与**线性**的更改数据，差分支持**线性**的查询与**常数级别**的更改。*如果需要查询与修改的时间复杂度一样低（降到$\log x$）,推荐使用[树状数组](https://oi-wiki.org/ds/fenwick/)或[线段树](https://oi-wiki.org/ds/seg/)。
